...
class grid(object):
    ...
    def convert_binary(self):
        ...
        newMatrix = [[] for x in range(len(binpix))]

        for i in range(len(binpix)):
            for j in range(len(binpix[i])):
                if binpix[i][j].color == (255,255,255):
...
def guess(binpix):
    ...
    predictions = model.predict(binpix)
    #print(predictions[0])
    predict = (np.argmax(predictions[0]))
    #print("Prediction : ", predict)
    window = Tk()
    window.withdraw()
    messagebox.showinfo("Prediction", "Number is a : " + str(predict))
    window.destroy()
